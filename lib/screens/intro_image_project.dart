...
import 'package:pimo/screens/image_item.dart';
import 'package:pimo/viewmodels/image_list_view_model.dart';
import 'package:provider/provider.dart';
import 'package:http/http.dart' as http;

class IntroImageProjectPage extends StatefulWidget {
  final int beginIndex;
  final int collectionId;
  final String modelId;
  const IntroImageProjectPage(
      {Key key, this.collectionId, this.beginIndex, this.modelId})
      : super(key: key);

  @override
  _IntroImageProjectPageState createState() => _IntroImageProjectPageState();
}

class _IntroImageProjectPageState extends State<IntroImageProjectPage> {
  deleteImageProject(int imageId) async {
    ...
    var response = await http.delete(
        Uri.parse('https://api.pimo.studio/api/v1/imageprojects'),
        headers: headers,
