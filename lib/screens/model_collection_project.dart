...
class ModelCollectionProject extends StatefulWidget {
  const ModelCollectionProject({Key key}) : super(key: key);
...
class _ModelCollectionProjectState extends State<ModelCollectionProject>
        ...
        child: FutureBuilder(
            ...
            builder: (context, snapshot) {
              return Scaffold(
                ...
                appBar: AppBar(
                  title: const Text('Bộ sưu tập dự án'),
                  backgroundColor: MaterialColors.mainColor,
                ),
                                  ...
                                  if (data.error == null) {
                                    return Consumer<
                                      ...
                                      builder: (ctx, data, child) =>
                                          ListView.builder(
                                        itemCount:
                                            data.listCollectionProject.length,
                                        itemBuilder:
                                            (BuildContext context, int index) {
                                          return _buildImageCollectList(
                                              (context),
                                              data.listCollectionProject[
                                                  index],
                                              index,
                                              snapshot.data.toString());
                                        },
  ...
  Widget _buildImageCollectList(
      BuildContext context, CollectionProjectViewModel collection, int index, String modelId) {
  ...
  void _showDialog(BuildContext context, String modelId) async {
    ...
    addCollectionProject(String name, String description, int projectId) async {
      //String id = (await FlutterSession().get('modelId')).toString();
      String accessToken = (await FlutterSession().get('jwt')).toString();
      var headers = {
        'Content-Type': 'application/json;charset=UTF-8',
      ...
      var msg = "Tạo thất bại";
        var response = await http.post(
            Uri.parse('https://api.pimo.studio/api/v1/collectionprojects'),
            headers: headers,
            body: jsonEncode({
              "name": name,
              "description": description,
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                await addCollectionProject(
                    nameController.text, desController.text, projectId);
              },
            ),
          ],
        );
